        ...
        $PrimaryButton.IsEnabled=$true
            $friendlyError=if($rawError -match 'runtime reservation|capacity|out of memory|CUDA.*memory'){
                ...
                if($freeMatch.Success){
                    ...
                    "NInfer needs about 14.6 GiB of free GPU memory, but only $([math]::Round($freeMiB/1024,1)) GiB is free. Stop TextGen, Harness, ComfyUI, Blender, or another GPU app, then retry the 32k profile."
                } else {
                    'NInfer could not reserve enough GPU memory for this profile. Stop TextGen, Harness, ComfyUI, Blender, or another GPU app, then retry the 32k profile.'
                }
