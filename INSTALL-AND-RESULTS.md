# AgentPort 1.7.5: RTX 4080 NInfer

1. Extract the release ZIP into a folder.
2. Open `AgentPort-v1.7.5-4080.exe`.
3. Press **Switch to NInfer**. If setup is needed, the same button installs it first. AgentPort stops TextGen, loads the compatible NInfer model, registers the **NInfer RTX 4080** provider, makes it the Harness default and opens Harness automatically.
4. Use **49k maximum** when maximum context matters and other GPU applications are closed. AgentPort switches its own backend and Harness processes automatically.
5. Open **MCP Connections** to paste standard `mcpServers` JSON, add a folder server, enable connections and restart Harness.
...
Build the executable from source with `go build -ldflags "-H=windowsgui" -o AgentPort-v1.7.5-4080.exe main.go`. The EXE extracts its bundled scripts into `%LOCALAPPDATA%/AgentPort/v1.7.5-4080`. Startup diagnostics are in that folder's `launcher.log`.
