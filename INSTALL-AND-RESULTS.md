# AgentPort 1.7.6: RTX 4080 NInfer

1. Extract the release ZIP into a folder.
2. Open `AgentPort-v1.7.6-4080.exe`.
...
Build the executable from source with `go build -ldflags "-H=windowsgui" -o AgentPort-v1.7.6-4080.exe main.go`. The EXE extracts its bundled scripts into `%LOCALAPPDATA%/AgentPort/v1.7.6-4080`. Startup diagnostics are in that folder's `launcher.log`.
