...
func main() {
    ...
    if err := os.MkdirAll(dir, 0755); err != nil {
        showError("AgentPort v1.6.1", "AgentPort could not create its runtime folder:\n\n"+err.Error())
        return
    }

    logPath := filepath.Join(dir, "launcher-v1.6.1.log")
    ps1 := filepath.Join(dir, "AgentPort-runtime-v1.6.1.ps1")
    ...
    if err := os.WriteFile(ps1, script, 0644); err != nil {
        ...
        showError("AgentPort v1.6.1", "AgentPort could not extract its embedded runtime.\n\n"+err.Error()+"\n\nLog: "+logPath)
        return
    }
    ...
    if _, err := os.Stat(powershell); err != nil {
        if found, lookErr := exec.LookPath("powershell.exe"); lookErr == nil {
            ...
            showError("AgentPort v1.6.1", "Windows PowerShell could not be found. AgentPort requires Windows PowerShell 5.1 to launch its desktop UI.\n\nLog: "+logPath)
            return
        }
    }
    ...
    if err != nil {
        ...
        showError("AgentPort v1.6.1 failed to start", "AgentPort hit a startup error instead of opening the UI.\n\n"+details+"\n\nFull log:\n"+logPath)
        return
    }
