...
function Invoke-NInferShell {
    ...
    try {
        $linuxPath=(& wsl.exe -d $Distro -- wslpath -u $path).Trim()
        if($LASTEXITCODE -ne 0){throw 'Cannot translate script path into WSL.'}
        $result=& wsl.exe -d $Distro -- bash $linuxPath
...
function Start-NInferService {
    ...
    $spec=if($Draft -eq 0){'--spec none'}else{"--spec mtp --draft-tokens $Draft --lm-head-draft"}
    $command=@"
set -euo pipefail
mkdir -p '$root/logs'
echo `$$ > '$pidFile'
...
"@
    ...
    $linuxLaunch=(& wsl.exe -d $Distro -- wslpath -u $launch).Trim()
