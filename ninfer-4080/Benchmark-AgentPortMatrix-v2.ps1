...
function Test-NInferReady {
    ...
    try{
        & wsl.exe -d $Distro -- bash -lc "test -x '$NInferRoot/ninfer-src/build-sm89/apps/ninfer-serve' -a -s '$NInferRoot/models/qwen3_8_27b_minq4.ninfer'" 2>$null | Out-Null
...
function Start-NInferBackend {
    ...
    try{ Wait-Api 240 | Out-Null }catch{
        $tail=& wsl.exe -d $Distro -- bash -lc "tail -n 100 '$NInferRoot/logs/ninfer-benchmark-v2.log' 2>/dev/null || true"
