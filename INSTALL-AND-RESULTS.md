# AgentPort 1.7.4: RTX 4080 NInfer

1. Extract the release ZIP into a folder.
2. Open `AgentPort-v1.7.4-4080.exe`.
3. Press **Use NInfer (Fast)**. If setup is needed, AgentPort offers to run it. Existing matching installations and model downloads are verified and reused.
4. Use **Use NInfer (49k)** when maximum context matters and other GPU applications are closed. AgentPort switches from its own TextGen and Harness processes automatically.
...
Build the executable from source with `go build -ldflags "-H=windowsgui" -o AgentPort-v1.7.4-4080.exe main.go`. The EXE extracts its bundled scripts into `%LOCALAPPDATA%/AgentPort/v1.7.4-4080`. Startup diagnostics are in that folder's `launcher.log`.
