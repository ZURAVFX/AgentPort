...
1. Download `AgentPort-Setup-3.0.3.exe` from the [latest release](https://github.com/ZURAVFX/AgentPort/releases/latest).
...
```text
Install AgentPort on this machine. Download AgentPort-Setup-3.0.3.exe from
...
- Fixes setup failing because bundled npm could not find `npm-prefix.js`, and automatically repairs incomplete bundled runtimes.
- Includes the earlier fix for the missing `proper-lockfile` dependency that prevented provider and Harness data from loading.
...
```powershell
certutil -hashfile AgentPort-Setup-3.0.3.exe SHA256
```
